...
119 个元素零差异（这个数随界面增删会变，以门禁最后一行为准）。**但这是一个演示级系统，不是可以接真实患者的系统。**
...
npm run audit         # **全面走查**（50 项）：每一处点下去坏不坏、卡不卡。需先起 dev
npm run audit:prod    # 同一份走查打线上（不重置任何状态）
...
## 5.1 数据库并发（2026-09-07）

SQLite 走 **WAL**，连接池 20+40。起因是一次压测：**120 并发 44 个超时**，
每个等满 30 秒 —— 打的是埋点与看板，两个都不调模型。修复后 **0.3s / 零失败**。

两个原因叠加：连接池只有 5+10 而 FastAPI 的线程池有 40；
SQLite 跑在回滚日志模式，读挡写、写挡读。
详见 [`docs/24-全面走查与并发修复.md`](docs/24-全面走查与并发修复.md)。

**运维上要记住一件事：数据不止一个文件。** WAL 模式下主库旁边有
`-wal` / `-shm`，且主库可能长期只有几 KB（实测主库 4 KB / WAL 984 KB）。
备份或搬迁必须三个一起拷，或先 `PRAGMA wal_checkpoint(TRUNCATE)` ——
只拷主库会拿到一个**合法但空**的数据库，而且不报错。
