---
name: grill-me
description: Pressure-test a plan, specification, ticket, or product decision through detailed questioning, then produce ambiguity and harness-constraint reports. Use only when explicitly invoked as $grill-me.
---
...
When the current repository is Doctor Agent, also read and apply
[`references/doctor-agent-product-grill.md`](references/doctor-agent-product-grill.md).
Use that extension to turn resolved product decisions into concrete harness constraints,
acceptance tests, and Mock fixtures. It extends the interview topics but does not replace
the five ambiguity dimensions or their exit report.
