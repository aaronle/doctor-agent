...
## 18. 问诊小结岗位 100% 降级（2026-09-03）

`interview_agent` **每一次都降级**，0ms 返回兜底 —— 也就是说「问诊小结」
这个岗位从上线起就没真正工作过，线上也一样。

**是顺手量出来的。** 做问诊提示浮框时需要 `analysis_gaps`，打了一次日志
才看见 `degraded: true`。没有任何告警、没有任何测试失败、界面上也看不出来 ——
兜底文案和真产出长得差不多，只是永远那几句。

### 模型自己发明了一种编码

两个相邻的 `list[str]` 字段被糊成了一个字符串，用 XML 标签分隔：

```json
{"key_points": "\n<item>三天前晨起发病</item>\n<item>右侧肢体无力</item>\n</gaps>\n"}
```

`gaps` 字段整个不见了 —— 它被塞进上一个字段的字符串里，连闭合标签都串了行。
**提示词里一个尖括号都没有**，是模型自发这么编的。

这和之前 summary 的「嵌套对象 → JSON 字符串」（§17）是同一类毛病，
只是换了种编码，所以既有的 `json.loads` 兜底认不出来：`<item>…` 不是合法 JSON。

### 两手都做，但分工不同

**提示词治本。** 加 `LIST_FIELD_CONVENTION`：数组字段每条是数组里的一个独立元素，
不要用 `<item>`、`-`、换行等任何标记在字符串内部分隔。实测 **4/4 全部正常**，
gaps 8–10 条且切题（「言语不清是构音障碍还是失语未问」这类）。

**运行时兜底，但划死边界。** `schemas.py` 里还原 `<item>…</item>`，
**只还原这一种明确编码** —— 不做按换行切、按 `-` 切之类的猜测性切分。
那一放开就是第二个 `extract_json_object`，正是 §17 这轮重构要消灭的东西。
配了一条反向测试钉住这个边界：换行分隔的字符串**必须**保持为单个字符串，不许被切开。

> 教训和 §17「差点丢掉的一个常量」是一对：
> **兜底要能认出自己不认识的东西**，而不是尽力猜一个。
> 猜错的产出会当成真产出用下去，比降级更难发现。

### 一个 Pydantic 的坑

正则最初写成类属性 `_ITEM_TAG = re.compile(...)`，运行时报「`Pattern` 对象没有
`findall`」—— Pydantic 把 `_` 开头的类属性变成 `ModelPrivateAttr`，
拿到的不是 compile 出来的那个对象。挪到模块级即可。

### 顺带：浮框只取前 4 条

模型一次给 8–10 条，那是**清单**；浮框要的是「顺手看一眼」。
条目一多医生就整个跳过，和一条不给同样的结果，还多占半个面板。
截断在前端（`HINT_LIMIT = 4`），后端照常全量返回。
