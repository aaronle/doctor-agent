...
### 新增接口要连带看一眼 Nginx 的读超时（2026-09-02 踩到）

编排层上线后 `/api/orchestration/*` 全部 504。应用侧的 `CALL_TIMEOUT_S` 是 300 秒，
但那条路径没有自己的 location 块，**继承的是 `location /` 的 60 秒** ——
Master 串六个 worker 实测 126 秒，必死。

这类问题健康检查发现不了，单测也发现不了：容器里一切正常，是 Nginx 提前把连接断了。
判据是**「这个接口的最坏耗时会不会超过它所在 location 的 `proxy_read_timeout`」**，
而不是「接口能不能通」。加接口时顺手回答这一句，比事后二分快得多。

三个慢路径当前的配额：

| 路径 | 应用上限 | Nginx | 实测 |
| --- | --- | --- | --- |
| `/api/emr/copilot/chat`、`/voice/turn` | —（SSE） | 300s | 逐块下发 |
| `/api/emr/report-summary/` | — | 180s | 约 20s |
| `/api/orchestration/` | 300s | **320s** | 单 worker 57s，Master 126s |

编排层给 320s 而不是 300s 是有意的：**让应用先超时**。应用超时会返回
「编排超过 300 秒未完成，已中止」，Nginx 超时只会返回一页什么都没说的 504 HTML。
谁知道得多，谁先超时。
